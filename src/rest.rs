...
use crate::utils::{SnowflakeID, http_client};
...
pub async fn accept_invite(token: &str, code: &str) -> Result<serde_json::Value, Box<dyn Error>> {
    let code = code
        .trim()
        .rsplit('/')
        .next()
...
pub async fn leave_guild(token: &str, guild_id: SnowflakeID) -> Result<(), Box<dyn Error>> {

...
use crate::send_struct::login_payload::{ClientState, D, PayloadLogin, Presence, Properties};
...
pub async fn _join_dm_call(socket: &mut Sink, channel_id: &str) -> Result<(), Box<dyn Error>> {
...
pub async fn _watch_stream(socket: &mut Sink, stream_key: &str) -> Result<(), Box<dyn Error>> {
    let payload = serde_json::json!({
        "op": 20,
...
pub async fn _delete_stream(socket: &mut Sink, stream_key: &str) -> Result<(), Box<dyn Error>> {
    let payload = serde_json::json!({
        "op": 19,
        "d": { "stream_key": stream_key }
    });
...
pub async fn _stream_ping(socket: &mut Sink, stream_key: &str) -> Result<(), Box<dyn Error>> {
    let payload = serde_json::json!({
        "op": 21,
        "d": { "stream_key": stream_key }
    });

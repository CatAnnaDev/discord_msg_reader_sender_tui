...
pub async fn handle_event(
    ...
    if key.code == KeyCode::Esc {
        ...
        if !s.watching_streams.is_empty() {
            ...
            if let Some(tx) = tx {
                let _ = tx.send(crate::tui::ingest::GatewayAction::StopStream);
            }
            return Ok(true);
        }
        s.should_quit = true;
                ...
                .filter(|g| *g != crate::tui::state::DM_GUILD_ID)
            {
                if s.members_requested.insert(g) {
                    if let Some(tx) = &s.gateway_tx {
                        let _ =
                            tx.send(crate::tui::ingest::GatewayAction::RequestMembers { guild: g });
    ...
    if key.code == KeyCode::Char('b') && key.modifiers.contains(KeyModifiers::CONTROL) {
        let mut s = state.write().await;
        if !s.watching_streams.is_empty() {
            let all_big = s.watching_streams.iter().all(|u| s.big_streams.contains(u));
            if all_big {
                s.big_streams.clear();
            } else {
...
async fn handle_settings_key(key: KeyEvent, state: &SharedState) -> Result<bool, Box<dyn Error>> {
    use crate::tui::state::{SETTINGS, SettingKind};
    let mut s = state.write().await;
    if s.settings_editing {
        match key.code {
...
async fn try_global_command(state: &SharedState, token: &str, raw: &str) -> bool {
    use crate::tui::ingest::GatewayAction;
    use crate::tui::state::{ChannelSummary, DM_GUILD_ID, PendingStream};
    let t = raw.trim();
        ...
        match rest::accept_invite(token, &code).await {
            Ok(v) => {
                let name = v
                    ...
                    .unwrap_or("server")
                    .to_string();
                state.write().await.set_status(format!("joined {name}"));
            }
            Err(e) => state.write().await.set_status(format!("join failed: {e}")),
        }
        return true;
    }
    ...
    if t == "/leave" {
        ...
        match guild {
            ...
            Some(g) => match rest::leave_guild(token, g).await {
                ...
                Err(e) => state.write().await.set_status(format!("leave failed: {e}")),
            },
        }
        return true;
    }
    ...
    if let Some(arg) = t.strip_prefix("/dm ") {
        ...
        match rest::create_dm(token, rid).await {
            ...
            Err(e) => state.write().await.set_status(format!("dm failed: {e}")),
        }
        return true;
    }
        ...
        match arg.trim().parse::<u64>() {
            Ok(uid) => {
                let (key, tx) = {
                    let mut s = state.write().await;
                    let sid = crate::utils::SnowflakeID::from_u64(uid);
                    if !s.watching_streams.contains(&sid) && s.watching_streams.len() < 4 {
    ...
    if t == "/call" {
        let (chan, guild, tx) = {
            let s = state.read().await;
            (s.current_channel, s.current_guild, s.gateway_tx.clone())
        };
        match (chan, guild, tx) {
            (Some(channel), Some(guild), Some(tx)) => {
                ...
                state.write().await.set_status("calling…".to_string());
            }
            _ => state
                .write()
                .await
...
async fn send_input(state: &SharedState, token: &str) -> Result<(), Box<dyn Error>> {
    {
        ...
        {
            let mut s = state.write().await;
            if s.input_history.last().map(|x| x.as_str()) != Some(raw.as_str()) {
    ...
    if let Some(path) = content.strip_prefix("/upload ") {
        ...
        state.write().await.set_status(format!("uploading {path}…"));

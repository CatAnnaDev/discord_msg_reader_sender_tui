...
use crate::receive_struct::gateway::{opcode, GatewayEnvelope};
...
use crate::send_struct::{
    _force_join_vocal, _join_dm_call, _leave_vocal, _request_guild_members,
    _stop_watching_stream, _watch_stream, payload,
};
...
async fn apply_event(
    ...
    match ev {
        ...
        DiscordEventMessage::VoiceStateUpdate { d } => {
            {
                ...
                if let Some(ch) = d.channel_id {
                    s.voice_members.entry(ch).or_default().push(
                        crate::tui::state::VoiceMember {
                            id: uid,
                            name,
        ...
        DiscordEventMessage::StreamServerUpdate { d } => {
            ...
            let key =
                d.get("stream_key").and_then(|v| v.as_str()).map(String::from);
            if let Some(key) = key {
                {
                    let mut s = state.write().await;
                    let e = s
                ...
                let is_ours =
                    state.read().await.golive_key.as_deref() == Some(&key);
                if is_ours {
        ...
        DiscordEventMessage::ChannelCreate { d } | DiscordEventMessage::ChannelUpdate { d } => {
            ...
            if let Some(c) = entry.iter_mut().find(|c| c.id == d.id) {
                *c = summary;
                ...
                entry.push(summary);
                entry.sort_by_key(|c| {
                    (c.parent_id.map(u64::from).unwrap_or(0), c.position, c.kind)
                });
            }
        }
            ...
            for p in &d.presences {
                    ...
                    .map(SnowflakeID::from_u64)
                {
                    if let Some(st) = p.get("status").and_then(|x| x.as_str()) {
                        s.presence.insert(
                            uid,
                            crate::tui::state::PresenceStatus::parse(st),
                        );
                    }
                }
            }
...
async fn maybe_start_voice(state: &SharedState) {
    ...
    let (tx, rx) = mpsc::unbounded_channel::<VoiceCommand>();
    {
        ...
        if let Some(old) = s.voice_cmd_tx.take() {
            crate::info!(
                "maybe_start_voice: disconnecting previous CALL voice manager"
            );
...
async fn maybe_start_stream(
    ...
    let signal = {
        ...
        match (
            ...
            (Some(token), Some(endpoint), Some(server), Some(uid))
                if p.uid != 0 =>
            {
                Some((
                    p.uid,
    ...
    let (tx, rx) = mpsc::unbounded_channel::<VoiceCommand>();
    {
        ...
        if let Some(old) = s.stream_conns.insert(buid, tx) {
            ...
            crate::info!(
                "maybe_start_stream: spawning watch conn for uid={buid}"
            );
        }
...
async fn apply_ready(d: ReadyData, state: &SharedState) {
    ...
    for g in &d.guilds {
        ...
        for vs in &g.voice_states {
            ...
            voice_members.entry(ch).or_default().push(crate::tui::state::VoiceMember {
                id: uid,
                name,
...
fn guild_channel_summaries(g: &Guild) -> Vec<ChannelSummary> {
    ...
    let mut categories: Vec<&ChannelSummary> =
        chans.iter().filter(|c| c.kind == 4).collect();
    ...
    for c in chans.iter().filter(|c| c.kind != 4 && c.parent_id.is_none()) {
                ...
                arr.iter()
                    .filter_map(|r| {
                        let name = r
                            ...
                            .to_string();
                        let count =
                            r.get("count").and_then(|x| x.as_u64()).unwrap_or(0) as u32;
                        Some((name, count))
                    })
                    .collect()
            })

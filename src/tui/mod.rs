...
async fn render_loop(
    ...
    let mut stream_views: HashMap<u64, stream_view::StreamView> =
        HashMap::new();
    ...
    loop {
        let watching_active;
        {
            ...
            for uid in &watching {
                if let Some((genr, w, h, rgb)) =
                    crate::voice::video::snapshot(*uid)
                {
                    if stream_gens.get(uid) != Some(&genr) {
                        if let Some(img) = image::RgbImage::from_raw(w, h, rgb)
                        {
                            ...
                            stream_views
                                .entry(*uid)
                                .or_insert_with(|| {
                                    stream_view::StreamView::new(&picker)
                                })
                                .update_frame(dynimg);
...

        {
            ...
            terminal.draw(|f| {
                ui::draw(f, &snap, &mut protocols, &mut stream_views)
            })?;
        }

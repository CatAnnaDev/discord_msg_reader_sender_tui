...
const HELP_LINES: &[(&str, &str)] = &[
    ...
    ("/watch <user id>", "watch someone's screen share (≤4, repeat)"),
    ("/big <uid> · Ctrl+B", "resize one stream big/PiP · Ctrl+B = all"),
    ("/unwatch <uid>", "stop one stream · /watch off or Esc = all"),
    ("/join /leave /dm /call", "invite · leave server · Create new DM · DM call"),
    ("q / Ctrl+C", "quit"),
    ...
    for t in toasts {
        ...
        let block = Block::default()
            ...
            .title(Span::styled(
                format!(
                    " ✦ {} ",
                    truncate(&t.title, (w as usize).saturating_sub(6))
                ),
                Style::default()
                    .fg(accent)
                    .add_modifier(Modifier::BOLD),
        ...
        match c {
            '`' => {
                ...
                spans.push(Span::styled(
                    code,
                    Style::default().fg(Color::Rgb(220, 220, 170)).bg(Color::Rgb(40, 40, 40)),
                ));
            }
            '*' if chars.peek() == Some(&'*') => {
                chars.next();
        ...
        if !m.reactions.is_empty() {
            ...
            for (name, count) in &m.reactions {
                rspans.push(Span::styled(
                    format!(" {name} {count} "),
                    Style::default().fg(Color::Black).bg(Color::Rgb(80, 80, 110)),
                ));

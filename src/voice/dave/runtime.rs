...
use super::libdave::{
    daveKeyRatchetDestroy, Decryptor, Encryptor, KeyRatchetHandle, Session,
};
use super::wire;
...
impl DaveRt {
    ...
    pub fn on_commit(&mut self, op29: &[u8]) -> bool {
        self.session.as_ref().map(|s| s.process_commit(op29)).unwrap_or(false)
    }
    ...
    pub fn debug_state(&self) -> String {
        ...
        rats.sort_unstable();
        let mut sp: Vec<(u32, u64)> =
            self.ssrc_uid.iter().map(|(s, u)| (*s, *u)).collect();
        sp.sort_unstable();
        format!(
    ...
    pub fn decrypt_video(&mut self, ssrc: u32, dave_frame: &[u8]) -> Result<Vec<u8>, i32> {
        ...
        let cap =
            d.max_plaintext_video(dave_frame.len()).max(dave_frame.len()) + 64;
        let mut out = vec![0u8; cap];

...
fn nv12_to_rgb(y: *const u8, ys: usize, uv: *const u8, uvs: usize, w: usize, h: usize) -> Vec<u8> {
...
fn nal_units(au: &[u8]) -> Vec<&[u8]> {
    ...
    while i + 3 <= au.len() {
        ...
        let sc4 =
            i + 4 <= au.len() && au[i] == 0 && au[i + 1] == 0 && au[i + 2] == 0 && au[i + 3] == 1;
        if sc3 || sc4 {
...
impl VtDecoder {
    ...
    fn ensure_session(&mut self, sps: &[u8], pps: &[u8]) -> bool {
        ...
        self.fmt = fmt;
        self.cb.refcon = Arc::as_ptr(&self.slot) as *const c_void as *mut c_void;
    ...
    pub fn feed(&mut self, au: &[u8]) -> bool {
        ...
        unsafe {
            ...
            if CMBlockBufferReplaceDataBytes(avcc.as_ptr() as *const c_void, bb, 0, avcc.len()) != 0
            {
                CFRelease(bb);
                return false;
            }

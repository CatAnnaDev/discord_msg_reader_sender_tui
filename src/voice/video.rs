...
static BIG: std::sync::atomic::AtomicBool =
...
pub fn spawn_decoder() -> std::sync::mpsc::Sender<(u64, Vec<u8>)> {
    ...
    std::thread::spawn(move || {
        ...
        let mut fail: u32 = 0;
        let mut q: std::collections::VecDeque<(u64, Vec<u8>)> =
        ...
        while let Ok(item) = rx.recv() {
            ...
            while let Some((uid, clear)) = q.pop_front() {
                ...
                if fed {
                    if !vt_ok {
                        ...
                        if !vt_logged {
                            vt_logged = true;
                            crate::info!(
                                "voice: VideoToolbox HW decode active"
                            );
                        }
                    }
                    vt_fed = true;
                ...
                if !vt_ok && !vt_logged {
                    vt_logged = true;
                    crate::info!(
                        "voice: VideoToolbox unavailable → openh264 software decode"
                    );
                }

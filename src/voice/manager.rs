    ...
    pub async fn run(
        ...
        if signal.watch_uid.is_some() || signal.broadcast {
            info!(
                ...
                if signal.broadcast { "broadcast" } else { "watch" },
                signal.watch_uid
            );
        ...
        let note_speaker = |env: &VoiceEnvelope,
        ...
        if let Some(wu) = signal.watch_uid {
            ...
            if let Err(e) =
                gateway::send_video_sink_wants(&mut sink, ready.ssrc).await
            {
        ...
        if signal.broadcast {
            ...
            match aes_gcm::Aes256Gcm::new_from_slice(&secret_key) {
                Ok(vc) => {
                    ...
                    let _ = gateway::send_speaking(&mut sink, 2, ready.ssrc)
                        .await;

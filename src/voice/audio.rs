...
fn transport_split(pkt: &[u8]) -> Option<(usize, usize)> {
    ...
    if pkt[0] & 0x10 != 0 {
        ...
        let ext_words =
            u16::from_be_bytes([pkt[base + 2], pkt[base + 3]]) as usize;
...
fn spawn_capture_thread(tx: mpsc::Sender<Vec<f32>>, want: Option<String>) {
    ...
    std::thread::spawn(move || {
        ...
        for (idx, dev) in candidates.into_iter().enumerate() {
            ...
            let Some(stream) = build_capture_stream(&dev, tx.clone(), peak.clone())
            else {
                continue;
            };
            ...
            if pk > 1.0e-5 || idx == last {
                info!(
                    "voice: capture device = {name} (peak={pk:.5}) — using it"
                );
                if pk <= 1.0e-5 {
                    error!(
...
async fn send_loop(
    ...
    crate::info!(
        ...
        dsp_tx.enabled, dsp_tx.hpf, dsp_tx.gate, dsp_tx.comp, dsp_tx.agc
    );
    ...
    while running.load(Ordering::Relaxed) {
        let Some(mut frame) = rx.recv().await else { break };
...
async fn recv_loop(
    ...
    let mut vrx: std::collections::HashMap<u32, crate::voice::video::VideoRx> =
        std::collections::HashMap::new();
    let mut vdec: std::collections::HashMap<
        u32,
        std::sync::mpsc::Sender<(u64, Vec<u8>)>,
    > = std::collections::HashMap::new();
    let mut vid_rx = 0u64;
    ...
    while running.load(Ordering::Relaxed) {
        ...
        if is_video {
            ...
            if vid_rx % 200 == 1 || (vid_rx < 4000 && dec.is_ok()) {
                ...
                crate::info!(
                    "voice recv video: rx={vid_rx} ssrc={ssrc} uid={uid:?} {st}"
                );
            }
        ...
        let opus: Vec<u8> = match dav {
            ...
            Err(_) => {
                ...
                let is_protocol =
                    plain.len() >= 3 && plain[plain.len() - 2..] == [0xFA, 0xFA];
                if is_protocol {
                    dave_fail += 1;
                    continue;
                }
